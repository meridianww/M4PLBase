...
namespace M4PL.Web.Controllers
{
	public class CommonController : MvcBaseController
	{
		...
		public PartialViewResult GetPopupNavigationTemplate(string strRoute)
		{
			...
			{
				if ( (route.IsActionPanel == false || route.IsCommentPanel == false))
				{
                    ...
                    if(CheckedData != null)
                    {
                        ...
                        {
                            route.IsActionPanel = true;
                            route.ActionTittle = CheckedData.Tittle;
                        }
                        if (CheckedData.GatewayTypeId == (int)JobGatewayType.Comment)
                        {
                            route.IsCommentPanel = true;
                        }
                        if (CheckedData.GatewayTypeId == (int)JobGatewayType.Gateway)
                        {
                            route.IsGatewayPanel = true;
                        }
                    }
                    else
                    {
