...
DevExCtrl.Ribbon = function () {
    ...
    var _onCommandExecuted = function (s, e, gridNameSuffix, appCbPanelName) {
        ...
        if (!M4PLCommon.CheckHasChanges.CheckDataChanges() || (route.Action === "Save")) {
            switch (route.Action) {
                case "FormView":
                    if (route.EntityName == "NAV Rate" || route.EntityName == "Gateway" || route.EntityName == "VendorProfile" || route.Controller == "NavRemittance" || route.Controller == "UserGuideUpload") {
            ...
            success: function (response) {
                if (response && response.status === true) {
                    ...
                    if (response.ownerName) {
                        ...
                        var gridCtrl = ASPxClientControl.GetControlCollection().GetByName(response.gridViewName);
                        if (gridCtrl != null && gridCtrl != undefined)
                            gridCtrl.ClearFilter();
                    }
                }
            }
        });
