USE [M4PL_DEV]
GO
/****** Object:  StoredProcedure [dbo].[GetPrgBillableLocations]    Script Date: 7/26/2019 11:19:27 AM ******/
SET ANSI_NULLS ON
GO
...
-- Execution:                 EXEC [dbo].[GetPrgBillableLocations]

-- ============================================= 
Create PROCEDURE [dbo].[GetPrgBillableLocations]
 @userId BIGINT,
 @roleId BIGINT,
 @orgId BIGINT,
 @entity NVARCHAR(100),
...
SET @TCountQuery = 'SELECT @TotalCount = COUNT(Id) FROM [dbo].[PRGRM042ProgramBillableLocations] (NOLOCK) '+ @entity 
...
SET @TCountQuery = @TCountQuery + ' WHERE [PblProgramID] = @parentId ' + ISNULL(@where, '')
...
SET @sqlCommand = @sqlCommand + ' FROM [dbo].[PRGRM042ProgramBillableLocations] (NOLOCK) '+ @entity

--Below to get BIGINT reference key name by Id if NOT NULL
SET @sqlCommand = @sqlCommand + ' LEFT JOIN [dbo].[PRGRM000Master] (NOLOCK) prg ON ' + @entity + '.[PblProgramID]=prg.[Id] '
SET @sqlCommand = @sqlCommand + ' LEFT JOIN [dbo].[VEND000Master] (NOLOCK) vend ON ' + @entity + '.[PblVendorID]=vend.[Id] '
...
SET @sqlCommand = @sqlCommand + ' WHERE '+@entity+'.[PblProgramID]=@parentId '+ ISNULL(@where, '')
...
IF((@recordId > 0) AND (((@isNext = 0) AND (@isEnd = 0)) OR ((@isNext = 1) AND (@isEnd = 0))))
   ...
   BEGIN  
	IF(ISNULL(@orderBy, '') <> '')
	 BEGIN
		SET @sqlCommand = @sqlCommand + ' AND ' + REPLACE(REPLACE(@orderBy, ' DESC', ''), ' ASC', '') + ' <= (SELECT ' + REPLACE(REPLACE(@orderBy, ' DESC', ''), ' ASC', '') + ' FROM [dbo].[PRGRM042ProgramBillableLocations] (NOLOCK) '+ @entity + ' WHERE ' + @entity + '.Id=' + CAST(@recordId AS NVARCHAR(50)) +') ' 
	 END
	ELSE
	 BEGIN
