...
namespace M4PL.APIClient.Common
{
	public class CommonCommands : ICommonCommands
	{
		...
		public IList<JobAction> GetJobAction(long jobId,string entity = null)
		{
			var content = _restClient.Execute(HttpRestClient.RestAuthRequest(Method.GET, string.Format("{0}/{1}", RouteSuffix, "JobAction"), ActiveUser).AddParameter("jobId", jobId).AddParameter("entity", entity)).Content;
