...
namespace M4PL.APIClient.Common
{
    public class CommonCommands : ICommonCommands
    {
        ...
        public IList<JobGatewayDetails> GetJobGateway(long jobId, string jobIds = null, bool IsMultiJob = false)
        {
            var content = _restClient.Execute(HttpRestClient.RestAuthRequest(Method.GET, string.Format("{0}/{1}", RouteSuffix, "GetJobGateway"), ActiveUser).AddParameter("jobId", jobId).AddParameter("jobIds", jobIds).AddParameter("IsMultiJob", IsMultiJob)).Content;
