...
{
	public class XcblSQLSerializer
	{
			[Flags]
			...
			static XcblSQLSerializer()
			{
			}

			public XcblSQLSerializer()
			{
				if (ConfigurationManager.ConnectionStrings[XcblConnectionConnectionStringName] == null)
					throw new InvalidOperationException(string.Format("Invalid default connection string name: {0}",
						XcblConnectionConnectionStringName));
				_connection =
					new SqlConnection(ConfigurationManager.ConnectionStrings[XcblConnectionConnectionStringName].ConnectionString);
				_command = _connection.CreateCommand();
			}

			public XcblSQLSerializer(ConnectionStringSettings connectionStringSetting)
			{
			...
			public XcblSQLSerializer(string connectionString)
			{
