        ...
        public Entities.Job.Job ToJobBasicDetailModelFromFarEyeData(FarEyeOrderDetails orderDetails, ref Entities.Job.Job jobDatatoUpdate, long programId, bool isASNRequest, List<SystemReference> systemOptionList)
        {
            ...
            if (orderDetails.item_list != null && orderDetails.item_list.Count > 0)
            {
                ...
                else
                {
                    ...
                    if(jobDatatoUpdate.JobType.Equals("Return", StringComparison.OrdinalIgnoreCase))
                    {
                        jobDatatoUpdate.JobQtyActual = qtyOrderedCount == 0 ? null : qtyOrderedCount;
                        jobDatatoUpdate.JobPartsActual = partsOrderedCount == 0 ? null : partsOrderedCount;
                        jobDatatoUpdate.JobServiceActual = jobServiceOrderCount == 0 ? null : jobServiceOrderCount;
                    }
                }
            }
