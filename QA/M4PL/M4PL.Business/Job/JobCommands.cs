...
namespace M4PL.Business.Job
{
	public class JobCommands : BaseCommands<Entities.Job.Job>, IJobCommands
	{
        ...
        public bool InsertJobGateway(BizMoblGatewayRequest bizMoblGatewayRequest, DateTime? gatewayACD = null)
        {
            var jobGatewayResult = _commands.InsertJobGateway(ActiveUser, bizMoblGatewayRequest, gatewayACD);
            bool result = jobGatewayResult != null && jobGatewayResult.Id > 0;
            if (result)
            {
                var jobDetails = _commands.GetJobByProgram(ActiveUser, bizMoblGatewayRequest.JobId, 0);
                JobGatewayCommands jobGatewayCommands = new JobGatewayCommands();
                jobGatewayCommands.ActiveUser = ActiveUser;
                jobGatewayCommands.PushDataToNav(bizMoblGatewayRequest.JobId, jobDetails.JobGatewayStatus, jobDetails.JobCompleted, jobDetails.JobTransitionStatusId, ActiveUser, jobDetails.CustomerId, jobGatewayResult.PgdGatewayNavOrderOption);
                bool isFarEyePushRequired = DataAccess.XCBL.XCBLCommands.InsertDeliveryUpdateProcessingLog(bizMoblGatewayRequest.JobId, M4PLBusinessConfiguration.ElectroluxCustomerId.ToLong());
                if (isFarEyePushRequired)
                {
                    FarEyeHelper.PushStatusUpdateToFarEye(bizMoblGatewayRequest.JobId, ActiveUser);
                }
            }
