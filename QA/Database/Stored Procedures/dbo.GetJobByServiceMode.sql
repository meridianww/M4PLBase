...
-- Description:               Get a Job data by Service Mode 
-- Execution                  EXEC [dbo].[GetJobByServiceMode] 2,14,1,'7400230134',20047 
-- =============================================        
CREATE PROCEDURE [dbo].[GetJobByServiceMode] @userId BIGINT
	,@roleId BIGINT
	,@orgId BIGINT
	,@JobServiceModeNumber VARCHAR(30)
	,@customerId BIGINT
AS
BEGIN TRY
	SET NOCOUNT ON;
	...
	WHERE Job.JobServiceMode = @JobServiceModeNumber AND Prg.PrgCustId = @customerId
		AND Job.StatusId = 1
