...
-- Description:               Get all Job EDIXcbl  
-- Execution:                 EXEC [dbo].[GetJobEDIXcblView]  
-- =============================================   
CREATE PROCEDURE [dbo].[GetJobEDIXcblView]  
	@userId BIGINT,  
	@roleId BIGINT,  
	@orgId BIGINT,  
	@entity NVARCHAR(100),  
...
SET @TCountQuery = 'SELECT @TotalCount = COUNT(Id) FROM [dbo].[JOBDL070ElectronicDataTransactions] (NOLOCK) '+ @entity   
...
SET @sqlCommand = @sqlCommand + ' FROM [dbo].[JOBDL070ElectronicDataTransactions] (NOLOCK) '+ @entity  
 ...
 BEGIN  
  IF((@isNext = 0) AND (@isEnd = 0))  
   BEGIN  
	IF(ISNULL(@orderBy, '') <> '')
	 BEGIN
		SET @sqlCommand = @sqlCommand + ' AND ' + REPLACE(REPLACE(@orderBy, ' DESC', ''), ' ASC', '') + ' <= (SELECT ' + REPLACE(REPLACE(@orderBy, ' DESC', ''), ' ASC', '') + ' FROM [dbo].[JOBDL070ElectronicDataTransactions] (NOLOCK) '+ @entity + ' WHERE ' + @entity + '.Id=' + CAST(@recordId AS NVARCHAR(50)) +') ' 
	 END
	ELSE
	 BEGIN
