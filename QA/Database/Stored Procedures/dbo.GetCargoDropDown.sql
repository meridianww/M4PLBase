...
-- Execution Sample: EXEC [dbo].[GetCargoDropDown] 'EN',1,'Cargo','JobCargo.Id,JobCargo.CgoPartNumCode,JobCargo.CgoTitle,JobCargo.CgoSerialNumber',1,10,null,null,null,null,null,1
-- Modified		 : 
--=================================================================================                      
...
BEGIN TRY
 ...
 SET @sqlCommand = '';
	 ...
	 BEGIN
		...
		SET @sqlCommand = @sqlCommand + ' WHERE 1=1 AND '+ @entity +'.JobID = '+ CONVERT(NVARCHAR(20), @parentId) +
		   ' (ISNULL('+ @entity +'.StatusId, 1) In (1,2)  OR ' + @primaryKeyName + '=' + @primaryKeyValue + ')';
	 ...
	 SET @sqlCommand += 'SELECT '+ @fields +' From [dbo].[JOBDL010Cargo] (NOLOCK) '+  @entity + ' WHERE 1=1 AND '+ @entity +'.JobID = '+ CONVERT(NVARCHAR(20), @parentId)
