...
-- =============================================
CREATE PROCEDURE [dbo].[BatchUpdateVendDcLocationContact] @userId BIGINT
	,@roleId BIGINT
	,@entity NVARCHAR(100)
	,@conOrgId BIGINT
	,@conCodeId BIGINT
...
IF (@isFormView = 0)
	BEGIN
		...
		SET [ConPrimaryRecordId] = CASE 
				WHEN (
						(@isFormView = 0)
						AND (@conVendDcLocationId = - 100)
						)
					THEN NULL
				ELSE ISNULL(@conVendDcLocationId, ConPrimaryRecordId)
				END
			,[ConItemNumber] = @updatedItemNumber
			,[ConTitle] = CASE 
				WHEN @conJobTitle = '#M4PL#'
					THEN NULL
				ELSE ISNULL(@conJobTitle, ConTitle)
				END
			,[ContactMSTRID] = CASE 
	...
	EXEC [dbo].[GetVendDcLocationContact] @userId
		,@roleId
		,@conOrgId
		,@id
END TRY
