...
BEGIN TRY                
 ...
 IF EXISTS ( SELECT 1 FROM [dbo].[JOBDL000Master] WHERE Id  = @jobId and StatusId =( SELECT ID FROM [SYSTM000Ref_Options] WHERE [SysOptionName] = 'Active' and [SysLookupCode] = 'Status'))
  BEGIN 
   ...
   SELECT @conCodeId =ID FROM  [dbo].[ORGAN010Ref_Roles]  WHERE  OrgRoleCode LIKE '%DRIVER%'  AND StatusId = 1 AND OrgID = @orgId
   ...
   SELECT @dcLocationId =  VDCL.ID FROM  [JOBDL000Master] JB
	...
	IF(ISNULL(@dcLocationId,0)<>0)
	   BEGIN
	          IF NOT  EXISTS (SELECT  TOP 1 1 FROM CONTC000Master WHERE   ConTypeId = @conTypeId AND  StatusId = 1 AND  ConUDF02 = @bizMoblContactID)
			  BEGIN
			
			   IF EXISTS (SELECT TOP 1 1 FROM   CONTC000Master WHERE ConFirstName = @firstName AND ConLastName =  @lastName AND ConTypeId = @conTypeId 
						AND StatusId = 1 AND ConUDF02 IS NULL) 
						BEGIN
							SELECT @masterContactId = ID FROM  CONTC000Master WHERE ConFirstName = @firstName AND ConLastName =  @lastName AND ConTypeId = @conTypeId 
							AND StatusId = 1 ORDER BY ID DESC
							UPDATE CONTC000Master SET [ConUDF02] = @bizMoblContactID WHERE  ID = (SELECT TOP 1  ID FROM   CONTC000Master WHERE  ConFirstName =  @firstName  AND ConLastName =  @lastName 	AND ConTypeId = @conTypeId 
							AND StatusId = 1 ORDER BY ID DESC)
						END
			  ...
			  BEGIN
			    ...
			    IF NOT EXISTS (SELECT TOP 1 1 FROM   CONTC000Master WHERE ConFirstName = @firstName AND ConLastName =  @lastName AND ConTypeId = @conTypeId 
						AND StatusId = 1 AND ConUDF02  = @bizMoblContactID ) 
							UPDATE CONTC000Master SET ConFirstName = @firstName, ConLastName =  @lastName WHERE  ConUDF02 = @bizMoblContactID
			  END
			   ...
			   AND  CM.ConFirstName = @firstName AND CM.ConLastName = @lastName AND CM.ConUDF02 = @bizMoblContactID AND JB.Id = @jobId   AND VDCL.StatusId = 1   AND PVL.StatusId =1
				  ...
				  ELSE
					  BEGIN
					    ...
					    UPDATE  [JOBDL000Master] SET  JobDriverId =  @masterContactId   WHERE Id=  @jobId 
						...
						VALUES (1, @masterContactId, 'VendDcLocationContact', @dcLocationId, 3, 'Driver', @conCodeId, @conTypeId, 1, NULL, NULL, NULL, 183, 'nfujimoto', '2020-07-28 00:46:37.003', NULL, NULL)

						SELECT Id,@locationCode AS locationCode,ConFirstName AS firstName,ConLastName AS lastName, ConUDF02 AS bizMoblContactID,@jobId AS jobId  FROM  CONTC000Master WHERE ID = @masterContactId
					END
	     END
   END
END TRY                
