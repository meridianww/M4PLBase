...
BEGIN TRY
	...
	SET @sqlCommand = @sqlCommand + ', CAST(0 AS BIT) IsIdentityVisible, Count(JobAdvanceReport.Id) JobCount, SUM(CASE 
			WHEN CAST(JobAdvanceReport.JobDeliveryDateTimeActual AS TIME) < ''17:00:00''
				THEN 1
			ELSE 0
			END) FivePMDeliveryWindow, SUM(CASE 
			WHEN ISNULL(JobAdvanceReport.JobOriginDateTimeActual, '''') <> ''''
    ...
    SET @sqlCommand += ' LEFT JOIN dbo.JOBDL020Gateways Gateway ON Gateway.JobId =' + @entity + '.Id' + ' AND GwyGatewayCode = ''Schedule'' AND Gateway.StatusId = 1'
	IF (ISNULL(@where, '') <> '')
	BEGIN
