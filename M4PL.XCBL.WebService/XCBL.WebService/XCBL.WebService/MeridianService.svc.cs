...
namespace XCBL.WebService
{
    ...
    {        
        ...
        public string Meridian_SendScheduleMessage(XCBLService xmldoc)
        {
            ...
            try
            {
                if (Meridian_AuthenticateUser(xmldoc))
                {

                    byte[] xdoc = xmldoc.XmlDocument;
                    ...
                    { 
                        ...
                        MeridianSystemLibrary.sysInsertTransactionRecord(xCblServiceUser.WebUsername, xCblServiceUser.FtpUsername, "Meridian_SendScheduleMessage", "1.0", "Error - The Select Statement for the ScheduleTypeCoded is incorrect", e.InnerException.ToString(), "filename", scheduleID);
                    }
                    ...
                    for (int i = 0; i < ds.Tables[MeridianGlobalConstants.XCBL_PURCHASE_ORDER_REFERENCE].Rows.Count; i++)
                    {
                        ...
                        if (dtTelphone.Rows.Count > 0)
                        {
                            ...
                            {
                                //TODO: Need to specify the filename
                                MeridianSystemLibrary.sysInsertTransactionRecord(xCblServiceUser.WebUsername, xCblServiceUser.FtpUsername, "Meridian_SendScheduleMessage", "1.1", "Error - The Contact Number are incorrect", e.InnerException.ToString(), "filename", scheduleID);
                            }
                        }
                    ...
                    { 
                        ...
                        MeridianSystemLibrary.sysInsertTransactionRecord(xCblServiceUser.WebUsername, xCblServiceUser.FtpUsername, "Meridian_SendScheduleMessage", "1.2", "Error - The CSV file cannot be created or closed.", e.InnerException.ToString(), Path.GetFileName(filePath), scheduleID);
                    ...
                    // Meridian_FTPUpload(xCblServiceUser.FtpServerUrl, xCblServiceUser.FtpUsername, xCblServiceUser.FtpPassword, filePath);
                    Meridian_FTPUpload(MeridianGlobalConstants.FTP_SERVER_URL, ftpUname, ftppassword, filePath);
                    status = MeridianGlobalConstants.MESSAGE_ACKNOWLEDGEMENT_SUCCESS;
                    return messageResponse.ToString();
                }
                else
            ...
            catch(Exception e)
            {
                ...
                MeridianSystemLibrary.sysInsertTransactionRecord(xCblServiceUser.WebUsername, xCblServiceUser.FtpUsername, "Meridian_SendScheduleMessage", "1.1", "Error - The XML Document cannot be parsed", e.InnerException.ToString(), "filename", scheduleID);
                return messageResponse.ToString();
            }         
        }

        private bool Meridian_AuthenticateUser(XCBLService xmldoc)
        {
            // This should use the XCBL_User xCblServiceUser variable for the username and password
            string uname = Encryption.Decrypt(xmldoc.Username, xmldoc.HashKey);
            string Password = Encryption.Decrypt(xmldoc.Password, xmldoc.HashKey);
            if (uname == "MeridanUser1" && Password == "Meridan123")
                return true;
            else
                return false;
        }

        private static void Meridian_FTPUpload(string ftpServer, string userName, string password, string filename)
        {
        ...
        public XCBLService Meridian_EncrpytCredentials(ref XCBLService xmldoc)
        {
            xmldoc.Username = Encryption.Encrypt(xmldoc.Username, xmldoc.HashKey);
            xmldoc.Password = Encryption.Encrypt(xmldoc.Password, xmldoc.HashKey);
            return xmldoc;
        }
    }
}
